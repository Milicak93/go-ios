...
import (
	...
	"github.com/Milicak93/go-ios/batterydiagnostics"
...
func printBatteryDiagnostics(device ios.DeviceEntry) {
	log.Debug("print battery diagnostics")
	batteryDiagnosticsService, err := batterydiagnostics.New(device)
	exitIfError("Starting diagnostics service failed with", err)
	values, err := batteryDiagnosticsService.AllValues()
	exitIfError("getting valued failed", err)
